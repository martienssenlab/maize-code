...
plot.peak.stats<-function(stattable, name) {
  ...
  table<-arrange(table, desc(Line), desc(Sample))
  
  plot<-ggplot(table, aes(Sample,Number,fill=Peak_type)) +
    geom_bar(stat="identity", position="dodge", color="black", show.legend = T) +
    labs(title=paste("Number of peaks in each TF ChIPseq sample of",analysisname), 
...
pdf(paste0("combined/plots/TF_ChIP_peak_stats_",analysisname,".pdf"), height=10, width=12)
