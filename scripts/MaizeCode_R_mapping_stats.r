...
plot.mapping.stats<-function(stattable, name) {
	table<-read.delim(stattable, header = TRUE, sep="\t") %>%  
		mutate(Label=paste(Line,Tissue,Sample,Rep)) %>% 
  ...
  plot1<-ggplot(table, aes(Label, Count, fill=Read_type)) +
    ...
    scale_fill_manual(values = c("Uniquely mapped"="springgreen","Multi-mapping"="yellow3",
                                 "Unmapped"="tomato3","Filtered"="red4")) +
  ...
  plot2<-ggplot(table, aes(Label, Count, fill=Read_type)) +
    ...
    labs(title = "", x="",y="", fill="") +
    scale_fill_manual(values = c("Uniquely mapped"="springgreen","Multi-mapping"="yellow3",
                                 "Unmapped"="tomato2","Filtered"="red4")) +
...
pdf(paste0("combined/plots/mapping_stats_",analysisname,".pdf"), height=8, width=8)
