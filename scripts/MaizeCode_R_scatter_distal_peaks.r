...
if ( grepl( "RNAseq", included_samples, fixed = TRUE) ) {
  ...
  plot1<-ggplot(tableTOT, aes(PeakQuality, RNAseq+0.1, color=labelRNAseq)) +
    geom_point() +
...
if ( grepl( "RAMPAGE", included_samples, fixed = TRUE) ) {
  ...
  plot2<-ggplot(tableTOT, aes(PeakQuality, RAMPAGE+0.01, color=labelRAMPAGE)) +
    ...
    scale_y_continuous(trans="log10",
      labels=scales::label_number(scale_cut = scales::cut_short_scale(), accuracy = 0.1)) +
    scale_x_continuous(trans="log10",
      labels=scales::label_number(scale_cut = scales::cut_short_scale(), accuracy = 1)) +
...
if ( grepl( "shRNA", included_samples, fixed = TRUE) ) {
  ...
  plot3<-ggplot(tableTOT, aes(PeakQuality, shRNA+0.1, color=labelshRNA)) +
    geom_point() +
...
if ( grepl( "RNAseq", included_samples, fixed = TRUE) && grepl( "shRNA", included_samples, fixed = TRUE) ) {
  plot5<-ggplot(tableTOT, aes(RNAseq+0.1, shRNA+0.1, size=PeakQuality, alpha=PeakQuality, color=PeakQuality)) +
  geom_point() +
