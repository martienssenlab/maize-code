...
do
	...
	if [ -e ${datatype}/chkpts/analysis_${name} ]; then
		...
		if [ ! -s ./TF/motifs/JASPAR2020_CORE_plants_non-redundant_pfms_meme.txt ]; then
			cp /grid/martienssen/data_nlsas_norepl/dropbox/maizecode/JASPAR2020_CORE_plants_non-redundant_pfms_meme.txt ./TF/motifs/JASPAR2020_CORE_plants_non-redundant_pfms_meme.txt
		fi
		datatype_list+=("${datatype}")
		new_tf_sample+=("${name}")
