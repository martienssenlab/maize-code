...
if [[ ${paired} == "PE" ]]; then
	...
	### Marking duplicates
 	if [[ ${mapparam} == "heavy" ]]; then
		STAR --runMode inputAlignmentsFromBAM --inputBAMfile mapped/map_${name}_Aligned.sortedByCoord.out.bam --bamRemoveDuplicatesType UniqueIdentical --limitBAMsortRAM 4000000000 --outBAMsortingBinsN 200 --outFileNamePrefix mapped/mrkdup_${name}_
	else
