	...
	qsub -N ${name} -V -cwd -sync y -pe threads 2 -l m_mem_free=2G -l tmp_free=50G -j y -o logs/analysis_${name}.log <<-'EOF1' &
		...
		do
			if [ -s motifs/${name}/meme/fimo_out_${num}/fimo.tsv ]; then
				...
				awk -v OFS="\t" -v n=$num 'NR > 1 {print $3,$4,$5,"motif_"n,$1}' motifs/${name}/meme/fimo_out_${num}/fimo.tsv | sort -k1,1 -k2,2n > motifs/temp_motifs_${name}.bed
		...
		do
			if [ -s motifs/${name}/meme2/fimo_out_${num}/fimo.tsv ]; then
				printf "Extracting peaks containing motif number ${num}\n"
				awk -v OFS="\t" -v n=$num 'NR > 1 {print $3,$4,$5,"motif_"n}' motifs/${name}/meme2/fimo_out_${num}/fimo.tsv | sort -k1,1 -k2,2n > motifs/temp_motifs_${name}.bed
