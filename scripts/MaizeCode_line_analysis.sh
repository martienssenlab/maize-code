...
do
	...
	if [[ ${test_moi} == "yes" ]] && [[ ${#tissue_bw_plus[@]} -ge 2 ]]; then
		...
		if [ -s combined/DEG/sorted_expression_${analysisname}_${tissue}.bed ]; then
			...
			if [[ ${ref} == "B73_v4" ]]; then
				bedtools sort -g ${ref_dir}/chrom.sizes -i ${regionfile} | awk -F"[:;]" -v OFS="\t" '{print $1,$2}' | awk -v OFS="\t" '{print $1,$2,$3,$5}' > combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed
			else
				bedtools sort -g ${ref_dir}/chrom.sizes -i ${regionfile} | awk -F"[=;]" -v OFS="\t" '{print $1,$2}' | awk -v OFS="\t" '{print $1,$2,$3,$5}' > combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed
			fi
			bedtools closest -a combined/peaks/temp_${analysisname}_${tissue}.bed -b combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed -D ref -t first -g ${ref_dir}/chrom.sizes | awk -v OFS="\t" '{if ($16>= 2000 && $15=="+") print $1,$2+$10,$12,$4,$5,$16,$14; else if ($16<= -2000 && $15=="-") print $1,$13,$2+$10,$4,$5,$15,$13}' | sort -k5,5nr > combined/peaks/distal_${analysisname}_${tissue}.bed
			bedtools closest -a combined/peaks/temp_${analysisname}_${tissue}.bed -b combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed -D ref -t first -g ${ref_dir}/chrom.sizes | awk -v OFS="\t" '{if ($16>= 2000 && $15=="+") print $1,$2,$3,$4,$5,$16,$14; else if ($16<= -2000 && $15=="-") print $1,$2,$3,$4,$5,$15,$13}' > combined/peaks/enhancers_distal_upstream_${analysisname}_${tissue}.txt
			bedtools closest -a combined/peaks/temp_${analysisname}_${tissue}.bed -b combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed -D ref -t first -g ${ref_dir}/chrom.sizes | awk -v OFS="\t" '{if ($16>= 2000 && $15=="-") print $1,$2,$3,$4,$5,$16,$14; else if ($16<= -2000 && $15=="+") print $1,$2,$3,$4,$5,$15,$13}' > combined/peaks/enhancers_distal_downstream_${analysisname}_${tissue}.txt
			bedtools closest -a combined/peaks/temp_${analysisname}_${tissue}.bed -b combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed -D ref -t first -g ${ref_dir}/chrom.sizes | awk -v OFS="\t" '{if ($16>0 && $17<2000 && $15=="+") print $1,$2,$3,$4,$5,$16,$14; else if ($16>-2000 && $16<0 && $15=="-") print $1,$2,$3,$4,$5,$15,$13}' > combined/peaks/enhancers_promoter_${analysisname}_${tissue}.txt
			bedtools closest -a combined/peaks/temp_${analysisname}_${tissue}.bed -b combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed -D ref -t first -g ${ref_dir}/chrom.sizes | awk -v OFS="\t" '{if ($16>0 && $17<2000 && $15=="-") print $1,$2,$3,$4,$5,$16,$14; else if ($16>-2000 && $16<0 && $15=="+") print $1,$2,$3,$4,$5,$15,$13}' > combined/peaks/enhancers_terminator_${analysisname}_${tissue}.txt
			bedtools closest -a combined/peaks/temp_${analysisname}_${tissue}.bed -b combined/peaks/temp_${analysisname}_${tissue}_no_expression.bed -D ref -t first -g ${ref_dir}/chrom.sizes | awk -v OFS="\t" '{if ($16==0) print $1,$2,$3,$4,$5,$15,$13}' > combined/peaks/enhancers_genic_${analysisname}_${tissue}.txt
		fi
