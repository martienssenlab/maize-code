...
if [[ "${total}" != "TEST" ]]; then
	...
	do
		...
		plotHeatmap -m combined/matrix/${matrix}_${analysisname}.gz -out combined/plots/${analysisname}_heatmap_${matrix}.pdf --sortRegions descend --sortUsing mean --samplesLabel ${all_labels[@]} --regionsLabel ${regionlabel} --colorMap 'seismic' --zMin ${mins[@]} --zMax ${maxs[@]} --yMin ${ymins[@]} --yMax ${ymaxs[@]} --interpolationMethod 'bilinear'
		printf "\nPlotting heatmap for ${matrix} matrix of ${analysisname} scaling by sample\n"
		plotHeatmap -m combined/matrix/${matrix}_${analysisname}.gz -out combined/plots/${analysisname}_heatmap_${matrix}_v2.pdf --sortRegions descend --sortUsing mean --samplesLabel ${all_labels[@]} --regionsLabel ${regionlabel} --colorMap 'seismic' --zMin ${mins2[@]} --zMax ${maxs2[@]} --yMin ${ymins2[@]} --yMax ${ymaxs2[@]} --interpolationMethod 'bilinear'
	done
...
do
	...
	if [[ ${rnaseq} == 1 ]] && [[ ${rampage} == 1 ]]; then
		...
		for type in genic promoter terminator distal_upstream distal_downstream
		do
			awk -v OFS="\t" 'NR>1 {print $1,$2,$3,$4,$5,$6,$9+$10,$11+$12,$7}' combined/peaks/complete_enhancers_${type}_${line}_${tissue}_${analysisname}.txt | sort -k6,6 -k7,7nr -k8,8nr > combined/peaks/sorted_enhancers_${type}_${line}_${tissue}_${analysisname}.txt			
			...
			regions_list_plus+=("combined/peaks/sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_plus.txt")
			regions_list_minus+=("combined/peaks/sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_minus.txt")
		...
		do
			...
			computeMatrix scale-regions -q --missingDataAsZero --skipZeros -R ${regions} -S ${bw[@]} -bs 10 -b 3000 -a 3000 -m 1000 -p ${threads} -o combined/matrix/regions_enhancers_${line}_${tissue}_${analysisname}_${strand}.gz
		done
		computeMatrixOperations rbind -m combined/matrix/regions_enhancers_${line}_${tissue}_${analysisname}_plus.gz combined/matrix/regions_enhancers_${line}_${tissue}_${analysisname}_minus.gz -o combined/matrix/regions_enhancers_${line}_${tissue}_${analysisname}.gz
		if [[ ${tefilebw} != "" ]]; then
		...
		plotProfile -m combined/matrix/regions_enhancers_${line}_${tissue}_${analysisname}.gz -out combined/plots/enhancers_${line}_${tissue}_${analysisname}_temp_profile.pdf --samplesLabel ${label_list[@]} --averageType mean --outFileNameData combined/matrix/values_enhancers_${line}_${tissue}_${analysisname}.txt
		rm -f combined/plots/enhancers_${line}_${tissue}_${analysisname}_temp_profile.pdf
		ymins=()
		ymaxs=()
		for sample in ${label_list[@]}
		do
			ymini=$(grep $sample combined/matrix/values_enhancers_${line}_${tissue}_${analysisname}.txt | awk '{m=$3; for(i=3;i<=NF;i++) if ($i<m) m=$i; print m}' | awk 'BEGIN {m=99999} {if ($1<m) m=$1} END {if (m<0) a=m*1.5; else a=m*0.5; print a}')
			ymaxi=$(grep $sample combined/matrix/values_enhancers_${line}_${tissue}_${analysisname}.txt | awk '{m=$3; for(i=3;i<=NF;i++) if ($i>m) m=$i; print m}' | awk 'BEGIN {m=-99999} {if ($1>m) m=$1} END {print m*1.5}')
			test=$(awk -v a=${ymini} -v b=${ymaxi} 'BEGIN {if (a==0 && b==0) c="yes"; else c="no"; print c}')
			if [[ "${test}" == "yes" ]]; then
				ymins+=("0")
				ymaxs+=("0.01")
		...
		computeMatrixOperations dataRange -m combined/matrix/regions_enhancers_${line}_${tissue}_${analysisname}.gz > combined/matrix/values_enhancers_${line}_${tissue}_${analysisname}.txt
		mins=()
		maxs=()
		totnb=${#label_list[*]}
		arr=$((totnb-2))
		for (( i=1; i<=${arr}; i++ ))
		do 
			mini=$(awk -v i=$i 'NR==(i+1) {print $5}' combined/matrix/values_enhancers_${line}_${tissue}_${analysisname}.txt)		
			maxi=$(awk -v i=$i 'NR==(i+1) {print $6}' combined/matrix/values_enhancers_${line}_${tissue}_${analysisname}.txt)
			test=$(awk -v a=${mini} -v b=${maxi} 'BEGIN {if (a==0 && b==0) c="yes"; else c="no"; print c}')
			if [[ "${test}" == "yes" ]]; then
				mins+=("0")
				maxs+=("0.005")
		...
		do
			...
			nb2=$(wc -l combined/peaks/sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_minus.txt | awk '{print $1}')
			regions_label="${type}_plus(${nb1}) ${type}_minus(${nb2})"
			computeMatrixOperations sort -m combined/matrix/regions_enhancers_${line}_${tissue}_${analysisname}.gz -R ${regions_list} -o combined/matrix/regions_enhancers_${line}_${tissue}_${type}_${analysisname}.gz
			printf "\nPlotting heatmap for ${line} ${tissue} ${type}\n"
			plotHeatmap -m combined/matrix/regions_enhancers_${line}_${tissue}_${type}_${analysisname}.gz -out combined/plots/enhancers_${type}_${line}_${tissue}_${analysisname}_sortedbyRNA.pdf --sortRegions keep --samplesLabel ${label_list[*]} --regionsLabel ${regions_label} --colorMap 'seismic' --interpolationMethod 'bilinear' --yMin ${ymins[@]} --yMax ${ymaxs[@]} --zMin ${mins[@]} --zMax ${maxs[@]}
			plotHeatmap -m combined/matrix/regions_enhancers_${line}_${tissue}_${type}_${analysisname}.gz -out combined/plots/enhancers_${type}_${line}_${tissue}_${analysisname}_sortedbyALL.pdf --sortRegions descend --sortUsing mean --samplesLabel ${label_list[*]} --regionsLabel ${regions_label} --colorMap 'seismic' --interpolationMethod 'bilinear' --yMin ${ymins[@]} --yMax ${ymaxs[@]} --zMin ${mins[@]} --zMax ${maxs[@]}
		done
...
#		for type in genic promoter terminator distal_upstream distal_downstream
#		do
#			awk -v OFS="\t" 'NR>1 {print $1,$2,$3,$4,$5,$6,$9+$10,$11+$12}' combined/peaks/complete_enhancers_${type}_${line}_${tissue}_${analysisname}.txt | sort -k6,6 -k7,7nr -k8,8nr > combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}.txt			
#			awk '$6=="+"' combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}.txt > combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_plus.txt
#			awk '$6=="-"' combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}.txt > combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_minus.txt
...
#			regions_label=("${type}_plus(${nb1})" "${type}_minus(${nb2})")
#			regions_sorted=("combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_plus.txt" "combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_minus.txt")
#			
#			for strand in plus minus
#			do
#				case "$strand" in
#					plus)	bw="${tissue_bw_plus[*]} ChIP/tracks/${regionname}.bw ${tefilebw}"
#						regions="combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_plus.txt";;
#					minus)	bw="${tissue_bw_minus[*]} ChIP/tracks/${regionname}.bw ${tefilebw}"
#						regions="combined/peaks/temp_sorted_enhancers_${type}_${line}_${tissue}_${analysisname}_minus.txt";;
#				esac
...
#				label_list="${tissue_labels[*]} Genes"
#			fi
#			printf "\nGetting scales for ${type} enhancers in ${tissue}\n"
#			plotProfile -m combined/matrix/regions_enhancers_${type}_${analysisname}_${tissue}.gz -out combined/plots/enhancers_${type}_${analysisname}_${tissue}_temp_profile.pdf --samplesLabel ${label_list[@]} --averageType mean --outFileNameData combined/matrix/values_enhancers_${type}_${analysisname}_${tissue}.txt
#			rm -f combined/plots/enhancers_${type}_${analysisname}_${tissue}_temp_profile.pdf
#			ymins=()
#			ymaxs=()
#			for sample in ${label_list[@]}
#			do
#				ymini=$(grep $sample combined/matrix/values_enhancers_${type}_${analysisname}_${tissue}.txt | awk '{m=$3; for(i=3;i<=NF;i++) if ($i<m) m=$i; print m}' | awk 'BEGIN {m=99999} {if ($1<m) m=$1} END {if (m<0) a=m*1.5; else a=m*0.5; print a}')
#				ymaxi=$(grep $sample combined/matrix/values_enhancers_${type}_${analysisname}_${tissue}.txt | awk '{m=$3; for(i=3;i<=NF;i++) if ($i>m) m=$i; print m}' | awk 'BEGIN {m=-99999} {if ($1>m) m=$1} END {print m*1.5}')
#				test=$(awk -v a=${ymini} -v b=${ymaxi} 'BEGIN {if (a==0 && b==0) c="yes"; else c="no"; print c}')
#				if [[ "${test}" == "yes" ]]; then
#					ymins+=("0")
#					ymaxs+=("0.01")
...
#				fi
#			done
#			computeMatrixOperations dataRange -m combined/matrix/regions_enhancers_${type}_${analysisname}_${tissue}.gz > combined/matrix/values_enhancers_${type}_${analysisname}_${tissue}.txt
#			mins=()
#			maxs=()
#			totnb=${#tissue_labels[*]}
#			for (( i=1; i<=${totnb}; i++ ))
#			do 
#				mini=$(awk -v i=$i 'NR==(i+1) {print $5}' combined/matrix/values_enhancers_${type}_${analysisname}_${tissue}.txt)		
#				maxi=$(awk -v i=$i 'NR==(i+1) {print $6}' combined/matrix/values_enhancers_${type}_${analysisname}_${tissue}.txt)
#				test=$(awk -v a=${mini} -v b=${maxi} 'BEGIN {if (a==0 && b==0) c="yes"; else c="no"; print c}')
#				if [[ "${test}" == "yes" ]]; then
#					mins+=("0")
#					maxs+=("0.005")
...
if [[ ${#uniq_shrna_tissue_list[*]} -ge 2 ]] && [[ ${tefilebw} != "" ]]; then
	...
	printf "\nIntersecting with TEs\n"
	bedtools intersect -a combined/shRNA/tmp5_shRNA_clusters_${analysisname}.bed -b combined/shRNA/${ref}_all_tes.bed -loj | awk -v OFS="\t" -v l=${line} 'BEGIN {printf "Line\tCluster_ID\tGene\tTE\tLabel\tLabelcombined\tGID\tTissues\tPeak_coordinates\n"} {if ($13==".") print l,$4,$9,"No",$9,$9,$8,$7,$1":"$2"-"$3; else if ($9 == "Intergenic") print l,$4,$9,$13,$13,$13,$8,$7,$1":"$2"-"$3; else print l,$4,$9,$13,$13,$13"_in_"$9,$8,$7,$1":"$2"-"$3}' > combined/shRNA/all_shRNA_clusters_in_genes_and_tes_${analysisname}.bed
