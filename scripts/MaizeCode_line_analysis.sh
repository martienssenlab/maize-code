...
do
	...
	do
		...
		if [[ ${tf} == "Yes" ]]; then
			...
			tfcol=$((colnb+4))
			bedtools intersect -wao -a combined/peaks/temp2_complete_enhancers_${type}_${line}_${tissue}_${analysisname}.txt -b combined/peaks/temp3_TFs_${analysisname}.txt | awk -v OFS="\t" -v c=${tfcol} -v a=${colnb} '{if ($c == ".") t="None"; else t=$20 ; for(i=1; i<=a; i++) {print $i}; print t}' > combined/peaks/temp4_complete_enhancers_${type}_${line}_${tissue}_${analysisname}.txt
